...
VMs must be powered off gracefully during the planned maintenance window. Before initiating the migration, verify that all prerequisites are met:

* Ensure the Shift Toolkit VM is part of the domain
* Ensure CIFS share is configured with appropriate permissions
* Verify the qtree used for migration or conversion has the correct security style
* Test VM creation using Hyper-V Manager from any Hyper-V host within the cluster and place the VHDx on the CIFS share
* Test VM creation from the Shift Toolkit VM by adding Hyper-V management tools using Programs and Features or PowerShell: `add-windowsfeature rsat-hyper-v-tools`
