...
* For preparing the VM, use local administrator for Windows OS and for Linux distributions, use a user who has permissions to execute commands without password prompt. Shift toolkit uses PowerShell direct for Windows VMs and SSH for Linux based VMs.
