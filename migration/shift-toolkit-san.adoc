...
After the VMs reside on the NFS datastore, the Shift Toolkit uses NetApp FlexClone technology to convert the VMs from VMware ESXi to Hyper-V. The converted VMs and their disks are placed on a CIFS share accessible by the Hyper-V host.

.Migrate back to SAN (Hyper-V environment)*

After conversion, use Hyper-V Storage Live Migration to move the converted VMs and their disks from the CIFS share to a SAN-enabled Cluster Shared Volume (CSV). This allows you to maintain your SAN infrastructure in the Hyper-V environment.
