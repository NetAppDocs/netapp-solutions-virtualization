...
summary: 
---
= Learn about migrating VMs between virtualization environments using NetApp Shift Toolkit
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
The NetApp Shift Toolkit is a standalone product designed to simplify and accelerate VM migrations across hypervisors, such as VMware ESXi, Microsoft Hyper-V, Oracle Linux Virtualization Manager, Redhat OpenShift, and others. It also supports disk-level conversions between various virtual disk formats.
...
* VMWare ESXi to Oracle Linux Virtualization Manager (OLVM)
* VMWare ESXi to Red Hat OpenShift Virtualization
* VMware ESXi to Red Hat OpenShift Virtualization using Migration Toolkit for Virtualization (MTV)
* VMware ESXi to Proxmox VE
...
* Microsoft Hyper-V to VMware ESX (virtual hard disk format [VHDX] to virtual machine disk [VMDK])
* VMware ESX to KVM compatible hypervisors (VMDK to QCOW2)
* VMware ESX to KVM compatible hypervisors (VMDK to RAW)

image::shift-toolkit-001.png["Overview of the NetApp Shift Toolkit"]
...
image::shift-toolkit-002.png["Diagram of how the NetApp Shift Toolkit works"]
...
[width=100%,cols="20%, 10%, 25%, 20%, 25%", frame=all, grid=all, options="header"]
|===
| Port | Protocol | Source | Destination | Purpose
| 443 | TCP | Shift toolkit node | VMware vCenter | VMware Inventory
| 443 | TCP | Shift toolkit node | VMware ESXi nodes | Invoke-vmscript dependency (routed)
| 443 | TCP | Shift toolkit node | Target Hypervisor | Target inventory
| 443 | TCP | Shift toolkit node | ONTAP system | ONTAP access
| 5985/5986 | HTTP | Shift toolkit node | Hyper-V Hosts | WinRM
|===
