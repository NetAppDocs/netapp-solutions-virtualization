...
* VMDKs can be configured in multiple ways: all VMDKs can reside on a single volume (eliminating the need for Storage vMotion), each VMDK can be placed on its own individual volume, or multiple VMDKs can be organized within a qtree inside a volume.
+
