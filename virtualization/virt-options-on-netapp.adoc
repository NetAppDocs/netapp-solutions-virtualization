...
* With ONTAP One, access all ONTAP features and robust security without licensing barriers. Pairing with NetApp Disaster Recovery further reduces cost.
...
* Leverage NetApp Disaster Recovery
...
NetApp offers an industry-leading SaaS-based disaster recovery solution that can significantly lower the costs and reduce complexity. There's no need to acquire and deploy expensive alternative infrastructure. 
...
For detailed information, see link:../migration/shift-toolkit-overview.html[Migrating virtual machines (VMs) between virtualization environments (Shift Toolkit)].
