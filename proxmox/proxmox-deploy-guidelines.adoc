...
---
= Deploy guidelines for Proxmox Virtual Environment with NetApp ONTAP storage
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
* Use link aggregation (LACP) for increased bandwidth and redundancy.
...
* Use Open vSwitch (OVS) over Linux Bridge when VLAN zones are configured.
...
* When using NFS, use the combination of nConnect, session trunking, and pNFS to optimize performance and availability.
