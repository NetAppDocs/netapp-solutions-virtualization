...
== Step 2: Add Proxmox Backup Server to Proxmox VE cluster
...
The Proxmox VE cluster automatically performs backups of the specified VMs and containers according to the defined schedule using Proxmox Backup Server as the storage target. The scheduled job configuration is stored in the /etc/pve/job.cfg file on the Proxmox VE host.
+
.Show example
[%collapsible]
====
...
== Step 4: Restore VMs and containers
...
== Step 5: Configure disaster recovery and monitoring

Protect backup data and manage multiple Proxmox environments. This includes replicating PBS datastores to another ONTAP system using SnapMirror for disaster recovery and monitoring multiple Proxmox VE and PBS instances using Proxmox Datacenter Manager.
...
. In the event of a disaster, mount the replicated PBS datastore on a secondary PBS instance. When adding the datastore in PBS, enable the "Reuse existing datastore" advanced option to avoid datastore reinitialization.
+
