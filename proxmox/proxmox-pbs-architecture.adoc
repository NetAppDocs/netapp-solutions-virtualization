...
PBS supports various storage backends, including local storage, NFS, iSCSI, and S3-compatible object storage. When integrated with NetApp ONTAP storage, PBS can leverage high-performance, scalable storage for backup datastores and offsite backup targets.
...
image::proxmox-pbs-architecture-001.png[Proxmox VE Workload Data Protection with PBS and NetApp ONTAP Architecture]
