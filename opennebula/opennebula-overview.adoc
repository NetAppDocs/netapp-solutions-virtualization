...
[.lead]
OpenNebula is an open-source cloud management platform for managing heterogeneous distributed data center infrastructures. It provides a simple but feature-rich solution to build private, public and hybrid clouds. OpenNebula supports virtual machines with KVM, linux containers with LXC, and CNCF certified Kubernetes based on SUSE Rancher. 
