...
summary: "Configure NFS networking on vSphere hosts in VMware Cloud Foundation workload domains to enable connectivity to ONTAP storage systems. You'll create distributed port groups with VLAN separation, configure uplink teaming for redundancy, and set up VMkernel adapters on each vSphere host to establish dedicated NFS paths for failover capabilities."
---

= Configure networking for NFS on vSphere hosts in a VCF workload domain
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
Perform the following steps on the workload domain cluster using the vSphere client. In this case vCenter Single Sign-On is being used so the vSphere client is common across the management and workload domains.
...
====
+
. Repeat this process for each vSphere host in the cluster.

== Step 2: Create a VMkernel adapter on each vSphere host
Create a VMkernel adapter on each vSphere host in the workload domain.
...
== What's next?
After you configure networking for NFS on all vSphere hosts in the workload domain, link:vmw-vcf-viwld-supp-nfs-vvols-storage.html[configure storage for NFS vVols].