...
* VCF management domain deployment is complete and the vSphere web client is accessible.
...
For multipathing and failover across multiple paths, create a minimum of two LIFs per storage node in separate Ethernet networks for all SVMs in iSCSI configurations. With ONTAP 9.19.1 and later, you can enable symmetric active-active multipathing option on SVMs on AFF. In ASA, symmetric active-active multipathing is enabled by default. 
...
== What's next?
After reviewing the deployment requirements, link:vmw-vcf-mgmt-supplemental-iscsi-svm-lifs.html[create the SVM and LIFs].
