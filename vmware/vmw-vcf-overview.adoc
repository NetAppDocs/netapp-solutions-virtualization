...
VCF is deployed to greenfield environments or to expand an existing vSphere environment to a full private cloud. A set of compute hosts form a vSphere cluster, a group of vSphere clusters form a VCF domain, and a group of VCF domains form a VCF instance. A fleet consists of one or more VCF instances. VCF private cloud environments are created using one or more fleets. Operation and Automation is handled at a fleet level, while the VCF instance is the unit of deployment. A VCF instance can be deployed in a single region or across multiple regions.
...
Namespaces are created with a dedicated VPC or shared VPC. A dedicated VPC is a VPC that is dedicated to a single namespace, while a shared VPC is a VPC that can be used by multiple namespaces. Subnets are created within a VPC where the workloads are deployed. Kubernetes clusters are created with its own namespace.
