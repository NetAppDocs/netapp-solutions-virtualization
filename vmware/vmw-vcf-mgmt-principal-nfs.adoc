...
* A vSphere cluster with at least 2 x ESXi hosts.
* NFS datastore mounted on one of the vSphere hosts with same NFS server and export path that will be used for the management domain. Better to have the NFS subnet accessible on management VLAN during the deployment of management domain.
