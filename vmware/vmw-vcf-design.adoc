...
This design blueprint is for deploying Management and Workload components in a Single vSphere Cluster with minimal resources. It supports VMFS and NFSv3 Principal Datastores and a simple deployment option with a two-node configuration. 
...
For more details on this design, refer to the link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/design/design-blueprints-for/infrastructure-modernization/vcf-fleet-in-a-single-site-with-minimal-footprint-blueprint.html[Broadcom Technical Documentation on VCF Fleet in a Single Site with Minimal Footprint].
...
Storage policy-based Management is used with vSphere Supervisor to host control VMs of VKS. Tags are centrally managed at VCF Ops. NetApp Trident CSI is used with VKS for application backup protection using native array features. When you use vSphere CSI, persistent volume details appear on vCenter as well as on VCF Automation.

For more details on this blueprint, refer to the link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/design/design-blueprints-for/infrastructure-modernization/vcf-fleet-in-a-single-site-blueprint.html[Broadcom Technical Documentation on VCF Fleet in a Single Site].
...
For NFS datastores, SnapMirror active sync for NAS (with ONTAP 9.19.1 & above) or ONTAP MetroCluster with vSphere Metro Storage Cluster ensures high availability. A mediator avoids split-brain scenarios and can now be hosted on NetApp Console.
...
ONTAP tools provide a UI to set up SnapMirror active sync relationships for iSCSI and FC. Storage Systems of both fault domains must be registered in ONTAP tools and SnapCenter Plug-in for VMware vSphere.
...
Trident Protect or NetApp Console Backup and Recovery for Kubernetes protect VKS Cluster Applications.

For more info, check the link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/design/design-blueprints-for/infrastructure-modernization/vcf-fleet-with-multiple-sites-in-a-single-region-blueprint.html[Broadcom Technical Documentation on VCF Fleet with Multiple Sites in a Single Region].
...
You can manage Disaster Recovery for VMs with VMware Protect and Recovery or NetApp Console Disaster Recovery. ONTAP tools offer the SRA (Storage Replication Adapter) to orchestrate storage operations with ONTAP.

[cols="4,2,2", options="header"]
|===
| Product Family       | SnapMirror active sync | SnapMirror active sync for NAS | MetroCluster
| ASA A-Series and C-Series | Yes                    | No                     | Yes
| AFX                   | No                     | Yes                     | NA
| AFF A-Series and C-Series | Yes                    | Yes                     | Yes
| FAS                  | No                     | Yes                     | Yes
|===
...
For more info, check the link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/design/design-blueprints-for/infrastructure-modernization/vcf-fleet-with-multiple-sites-across-multiple-regions-blueprint.html[Broadcom Technical Documentation on VCF Fleet with Multiple Sites Across Multiple Regions].
...
ASA, AFX, AFF, and FAS support this design option.
...
For more information, see the link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/design/design-blueprints-for/infrastructure-modernization/vcf-fleet-with-multiple-sites-in-a-single-region-plus-additional-region-blueprint.html[Broadcom Technical Documentation on VCF Fleet with Multiple Sites in a Single Region plus Additional Regions].