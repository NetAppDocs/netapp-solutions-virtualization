...
. In the vSphere client, navigate to one of the ESXi hosts in the workload domain cluster. From the *Configure* tab, click *Storage Adapters* in the menu. From the *Add Software Adapter* drop-down menu, select *Add NVMe over TCP adapter*.
+ 
...
* Fill in an IP address for one of the SVM logical interfaces on the same network as the physical adapter assigned to this NVMe/TCP adapter. 
...
== What's next?
After configuring networking, you link:vmw-vcf-viwld-supp-nvme-storage.html[configure storage for NVMe vVols].
