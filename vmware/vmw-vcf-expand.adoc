...
summary: "VMware Cloud Foundation (VCF) supports a wide range of supplemental storage options to expand storage on VCF management domains and workload domains."
---
...
[.lead]
VMware Cloud Foundation (VCF) supports a wide range of supplemental storage options to expand storage on VCF management domains and workload domains.
...
You can use the following protocols to expand storage in VCF management domains and workload domains. Choose the best option for your environment to seamlessly integrate supplemental storage into your VCF deployment.
...
iSCSI is widely used for VMFS datastores and supports advanced ONTAP features, such as snapshots and cloning. ONTAP 9.19.1 and later supports symmetric active-active configuration on AFF systems too. Initially that feature was only available on ASA systems.
...
ONTAP 9.19.1 and later supports symmetric active-active configuration on AFF systems too. Initially that feature was only available on ASA systems.
...
* *Scalability:* ONTAP's architecture allows NFS to scale efficiently, supporting growing data needs without significant changes to the infrastructure. FlexGroup along with pNFS can be used to scale NFS datastores to petabyte levels.
...
.NVMe-oF
Modern protocol that delivers high performance and low latency over standard Ethernet networks using TCP/IP. NVMe-oF (NVMe over Fabrics) is ideal for demanding applications and large-scale data operations, providing scalability and cost efficiency without requiring specialized hardware.
...
* *Cost Effective:* Runs over standard Ethernet switches and is encapsulated inside TCP datagrams. No special equipment required to implement. NVMe-oF over TCP is commonly used.
