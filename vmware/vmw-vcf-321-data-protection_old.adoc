...
= Configure 3-2-1 data protection for VMware with SnapCenter plug-in for VMware vSphere and NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Configure a 3-2-1 data protection strategy for VMware environments using the SnapCenter plug-in for VMware vSphere and NetApp Backup and Recovery. This procedure includes setting up backups of VMs and datastores on primary and secondary ONTAP clusters, configuring the plug-in and NetApp Console, and managing data replication to cloud or offsite storage for reliable recovery.
...
This solution document covers a 3-2-1 backups solution using SnapCenter Plug-in for VMware vSphere (SCV) to create primary and secondary backups of our on-premises virtual machines and NetApp Backup and Recovery for virtual machines to backup a copy of our data to cloud storage or StorageGRID.
...
* Backup and restore of on-premises virtual machines and datastores, hosted on ONTAP clusters, and backed up to object storage using NetApp Backup and Recovery for virtual machines.
...
== NetApp Backup and Recovery for virtual machines

NetApp Backup and Recovery is a cloud based tool for data management that provides a single control plane for a wide range of backup and recovery operations across both on-premises and cloud environments. Part of the NetApp Backup and Recovery is a feature that integrates with the SnapCenter Plugin for VMware vSphere (on-premises) to extend a copy of the data to object storage in the cloud. This establishes a third copy of the data offsite that is sourced from the primary or secondary storage backups. NetApp Backup and Recovery makes it easy to set up storage policies that transfer copies of your data from either of these two on-prem locations.

Choosing between the primary and secondary backups as the source in NetApp Backup and Recovery will result in one of two topologies being implemented:

*Fan-out Topology* – When a backup is initiated by the SnapCenter Plug-in for VMware vSphere, a local snapshot is immediately taken. SCV then initiates a SnapMirror operation that replicates the most recent snapshot to the Secondary ONTAP cluster. In NetApp Backup and Recovery, a policy specifies the primary ONTAP cluster as the source for a snapshot copy of the data to be transferred to object storage in your cloud provider of choice.

image:bxp-scv-hybrid-001.png[Fan-out topology,800,418]

*Cascading Topology* – Creating the primary and secondary data copies using SCV is identical to the fan-out topology mentioned above. However, this time a policy is created in NetApp Backup and Recovery specifying that the backup to object storage will originate from the secondary ONTAP cluster.

image:bxp-scv-hybrid-002.png[Cascading topology]

NetApp Backup and Recovery can create backup copies of on-premises ONTAP snapshots to AWS Glacier, Azure Blob, and GCP Archive storage.
...
[%collapsible%open]
=====
This list provides the high level steps necessary to configure this solution and execute backup and restore operations from SCV and NetApp Backup and Recovery:
...
. Configure NetApp Backup and Recovery for virtual machines
...
. Restore virtual machines from object storage using NetApp Backup and Recovery.
=====
...
. NetApp Console agent installed for cloud provider used for object storage backups.
. Virtual machines to be backed are on NFS datastores residing on the primary ONTAP storage cluster.
. Network connectivity between the NetApp Console agent and on-premises ONTAP storage cluster management interfaces.
. Network connectivity between the NetApp Console agent and on-premises SCV appliance VM and between the agent and vCenter.
...
In this solution, we provide detailed instructions for deploying and validating a solution that utilizes SnapCenter Plug-in for VMware vSphere, along with NetApp Backup and Recovery, to perform the backup and recovery of Windows and Linux virtual machines within a VMware vSphere cluster located in an on-premises data center. The virtual machines in this setup are stored on NFS datastores hosted by an ONTAP A300 storage cluster. Additionally, a separate ONTAP A300 storage cluster serves as a secondary destination for volumes replicated using SnapMirror. Furthermore, object storage hosted on Amazon Web Services and Azure Blob were employed as targets for a third copy of the data.

We will go over creating SnapMirror relationships for secondary copies of our backups managed by SCV and configuration of backup jobs in both SCV and NetApp Backup and Recovery.
...
For detailed information on NetApp Backup and Recovery refer to the https://docs.netapp.com/us-en/data-services-backup-recovery/index.html[NetApp Backup and Recovery documentation].
...
Setting up SnapMirror relationships can be done in the NetApp Console where many of the steps are automated, or it can be done using System Manager and the ONTAP CLI. All of these methods are discussed below.

=== Establish SnapMirror relationships with the NetApp Console

The following steps must be completed from the NetApp Console:

.Replication setup for primary and secondary ONTAP storage systems
[%collapsible%open]
==== 
Begin by logging into the NetApp Console and navigating to management.
...
+
NOTE: Snapshot labels are useful in that they can be used to enact policies with a specific retention period for the SnapVault copies replicated to the secondary ONTAP cluster. When SCV is used with NetApp Backup and Recovery, the Snapshot label field must either be blank or [underline]#match# the label specified in the backup policy.
...
+
NOTE: NetApp Backup and Recovery does not currently support backing up VMs with VMDKs that span multiple datastores.
...
+
NOTE: When backing up SCV managed snapshots to object storage using NetApp Backup and Recovery, each resource group can only be associated with a single policy.
...
[%collapsible%open]
=====
In this final step, run a backup job and monitor its progress. At least one backup job must be successfully completed in SCV before resources can be discovered from NetApp Backup and Recovery.
...
=== Configure Backups to Object Storage in NetApp Backup and Recovery

For NetApp Console to manage the data infrastructure effectively, it requires the prior installation of an agent. The NetApp Console agent executes the actions involved in discovering resources and managing data operations.

For more information on the NetApp Console agent refer to https://docs.netapp.com/us-en/console-setup-admin/concept-agents.html[Learn about NetApp Console agents] in the NetApp Console documentation.
...
To configure NetApp Backup and Recovery to backup data managed by SCV on-premises, complete the following steps:

.Add working environments to systems under management
[%collapsible%open]
=====
The first step is to add the on-premises ONTAP storage systems to the NetApp Console

. From the systems under management select *Add* to begin.
+
...
. From NetApp Console left-hand menu selection *Protection > Backup and Recovery > Virtual Machines*
+
...
For more information on creating policies refer to https://docs.netapp.com/us-en/bluexp-backup-recovery/prev-vmware-policy-create.html[Create a policy to back up datastores].
...
For more information refer to https://docs.netapp.com/us-en/bluexp-backup-recovery/prev-vmware-backup-aws.html[Back up datastores to Amazon Web Services].
...
For more information refer to https://docs.netapp.com/us-en/bluexp-backup-recovery/prev-vmware-restore.html[Restore virtual machines data from the cloud].
...
// NetApp Solutions restructuring (jul 2025) - renamed from vmware/vmw-vcf-scv-321.adoc
