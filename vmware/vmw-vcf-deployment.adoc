...
[.lead]
Deploy, converge or import, upgrade VMware Cloud Foundation 9 environments with ONTAP. Learn how to set up new VCF 9.1 environments, converge existing vCenter instances as Management Domain or import existing vCenter instances as Workload Domain, and upgrade earlier VCF deployments. 

For more details, refer to the link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/deployment/overview-of-deploy--converge--and-upgrade.html[Broadcom link for VCF 9.1 deployment paths].

== Deploy a new VCF 9 instance
Use this workflow to deploy a clean VMware Cloud Foundation (VCF) 9.1 environment. After deployment, you can migrate workloads or begin provisioning applications and provide infrastructure services.

.Steps
. Follow the link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/deployment/deploying-a-new-vmware-cloud-foundation-or-vmware-vsphere-foundation-private-cloud-/preparing-your-environment.html[Broadcom VCF 9 deployment steps].
...
. Deploy the ONTAP NFS VAAI VIB and utilize that server for the cluster image. For example: `esxcli software vib install -d /NetAppNasPlugin2.0.1.zip`. (Download the ZIP from the NetApp Support Site.)
...
[start=3]
. After successful deployment of the link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/deployment/deploying-a-new-vmware-cloud-foundation-or-vmware-vsphere-foundation-private-cloud-/deploy-a-new-vcf-fleet-or-a-new-vcf-instance.html[VCF Fleet] phase, complete the following:
.. Deploy ONTAP tools
...
.. Deploy the NetApp Console agent
* link:https://docs.netapp.com/us-en/console-setup-admin/concept-agents.html[Review what you can do without a Console agent].
* link:https://docs.netapp.com/us-en/console-setup-admin/concept-modes.html#overview[Agent deployment modes].
.. Use NetApp Backup and Recovery
* link:https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-vmware-protect-overview.html[Protect VM workloads].
* link:https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-kubernetes-protect-overview.html[Protect VKS workloads].

. After you link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/building-your-private-cloud-infrastructure/working-with-workload-domains/import-an-existing-vcenter-to-create-a-workload-domain.html[import vCenter as a workload domain] in the VCF instance, complete the following:
...
.Steps
. Review the link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/deployment/converging-your-existing-vsphere-infrastructure-to-a-vcf-or-vvf-platform-/supported-scenarios-to-converge-to-vcf.html[supported scenarios to converge to VCF].
...
Upgrade an earlier VCF deployment to version 9.1 using the standard upgrade process. The outcome is a VCF environment running version 9.1 with upgraded management and workload domains.

.Prerequisites
* Back up the management domain and workload domains.
* Verify compatibility of ONTAP tools and SnapCenter Plug-in with VCF 9.1. Follow the link:https://imt.netapp.com/imt/#welcome[Interoperability Matrix] to link:https://docs.netapp.com/us-en/ontap-tools-vmware-vsphere-10/upgrade/upgrade-ontap-tools.html[upgrade ONTAP tools] and link:https://docs.netapp.com/us-en/sc-plugin-vmware-vsphere/scpivs44_upgrade.html[SnapCenter Plugin for VMware vSphere] that are supported for VCF 9.1.


.Steps
. Upgrade the VCF management domain. See link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/deployment/upgrading-cloud-foundation.html[Upgrade VCF Management Domain to VCF 9.1] for instructions.
. Upgrade any VCF 5.x workload domains. See link:https://techdocs.broadcom.com/us/en/vmware-cis/vcf/vcf-9-0-and-later/9-1/lifecycle-management/upgrade-workload-domains-to-vcf-5-2.html[Upgrade VCF 5.x Workload Domain to VCF 9.1] for instructions.
