...
summary: "Create a Storage Virtual Machine (SVM) with multiple Logical Interfaces (LIFs) to provide NVMe/TCP connectivity for VMware Cloud Foundation workload domains. You'll configure the SVM with NVMe protocol support and set up multiple LIFs across separate Ethernet networks to enable multipathing and failover for optimal performance and availability."
---
= Create SVM and LIFs and the NVMe namespace for NVMe/TCP datastores in a VMware Cloud Foundation workload domain
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
. From ONTAP System Manager, navigate to *Storage VMs* in the left-hand menu and click *+ Add*. 
+
.Show example
[%collapsible]
====
...
====
+
. In the *Add Storage VM* wizard, enter a *Name* for the SVM, select the *IP Space* and then, under *Access Protocol*, click the *NVMe* tab and check the box to *Enable NVMe/TCP*.
+
.Show example
[%collapsible]
====
...
====
+
. In the *Network Interface* section, enter the *IP address*, *Subnet Mask*, and *Broadcast Domain and Port* for the first LIF. For subsequent LIFs, you can either use individual settings or enable the checkbox to use common settings across all remaining LIFs.
+
...
NVMe namespaces are analogous to LUNs for iSCSi or FC. You must create the NVMe Namespace before a VMFS datastore can be deployed from the vSphere Client. 

To create the NVMe namespace, get the NVMe Qualified Name (NQN) from each vSphere host in the cluster. ONTAP uses the NQN to provide access control for the namespace. 

.Steps

. Open an SSH session with a vSphere host in the cluster to obtain its NQN. Use the following command from the CLI:
+
[source, cli]
...
+
. Record the NQN for each vSphere host in the cluster.
+
...
+
. In the *Host NQN* section, create a comma separated list of the NQN's previously collected from the vSphere hosts that will be accessing the namespaces. 
+
...
== What's next?
After creating the SVM and LIFs, link:vmw-vcf-viwld-supp-nvme-network.html[configure networking for NVMe/TCP (NVMe/TCP)].
