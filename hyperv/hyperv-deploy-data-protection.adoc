...
---
= Deploy Microsoft Hyper-V on NetApp storage
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Deploy Microsoft Hyper-V virtual machines using ONTAP storage-based solutions and third-party backup integration. This process includes using ONTAP Snapshot copies and FlexClone technology for rapid backup and restore operations, configuring CommVault IntelliSnap for enterprise backup management, and implementing SnapMirror replication for backup and disaster recovery across sites. 

Learn to address unique Hyper-V backup considerations like disk ID conflicts in clustered environments and optimize data protection for standalone hosts and Hyper-V clusters.
...
+
image:hyperv-deploy-009.png[Image of the Array Properties dialog]
...
+
image:hyperv-deploy-010.png[Image of configuring the SVM as an array management entry]

. Click on Advanced (as shown in the below graphics) and select "Enable IntelliSnap" checkbox.
+
image:hyperv-deploy-011.png[Image displaying the Enable IntelliSnap option]

For detailed steps about configuring the array, see link:https://documentation.commvault.com/v11/commcell-console/configuring_netapp_array_using_array_management.html[Configuring NetApp Array] and link:https://documentation.commvault.com/v11/commcell-console/configure_storage_virtual_machine_on_netapp_storage_array.html[Configuring Storage Virtual machines on NetApp Arrays]
...
+
image:hyperv-deploy-012.png[Image displaying the discovery of hyper-v nodes]

. Click Next and the Save.
+
image:hyperv-deploy-013.png[Image showing the results of the previous step]
...
+
image:hyperv-deploy-014.png[Image showing the selection of VMs to protect]
+
...
For detailed steps about configuring the array, see link:https://documentation.commvault.com/2023e/software/guided_setup_for_hyper_v.html[Guided Setup for HyperV].
...
+
image:hyperv-deploy-015.png[IMage showing the dialog to select VMs to be backed up]
...
+
image:hyperv-deploy-016.png[Image displaying the restore points for a backup]
+
...
+
image:hyperv-deploy-017.png[Image showing the restore types for a backup]

. Select Guest files or Full virtual machine depending on the selection and trigger the restore.
+
image:hyperv-deploy-018.png[Image displaying the options for the restore]

For detailed steps for all supported restore options, see link:https://documentation.commvault.com/2023e/software/restores_for_hyper_v.html[Restores for Hyper-V].
