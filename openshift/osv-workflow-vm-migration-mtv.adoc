...
This section describes how to use the Migration Toolkit for Virtualization (MTV) to migrate virtual machines from VMware to OpenShift Virtualization running on OpenShift Container Platform and integrated with NetApp ONTAP storage using Trident. 
...
=== On VMware
* A RHEL 9 VM using RHEL 9.3 with the following configurations was installed:
** CPU: 2, Memory: 20 GB, Hard disk: 20 GB 
** User credentials: root user and admin user 
...
These commands ensure that the server can start in the VM in OpenShift Virtualization after migration.

** Added 2 databases, 1 table, and 1 row to the table. Refer to link:https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/9/html/configuring_and_using_database_servers/using-postgresql_configuring-and-using-database-servers#configuring-postgresql_using-postgresql[here] for instructions on installing PostgreSQL server on RHEL and creating database and table entries.
...
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19#Virtualization[OpenShift Virtualization]
...
You can refer to link:https://access.redhat.com/documentation/en-us/migration_toolkit_for_virtualization/2.5/html/installing_and_using_the_migration_toolkit_for_virtualization/migrating-vms-web-console#mtv-ui_mtv[here] to start using the user interface for various tasks.

==== Create source provider

To migrate the RHEL VM from VMware to OpenShift Virtualization, you need to first create the source provider for VMware. Refer to the instructions link:https://access.redhat.com/documentation/en-us/migration_toolkit_for_virtualization/2.5/html/installing_and_using_the_migration_toolkit_for_virtualization/migrating-vms-web-console#adding-providers[here] to create the source provider.
...
The host cluster is automatically added as the destination provider.
...
* A storage mapping to map the source datastore to the target storage class. For this, you can choose the ontap-san storage class.
...
Click *Start* to run through a sequence of steps and complete the migration of the VM.
...
To manage the virtual machines, see link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/virtualization/managing-vms[the OpenShift Virtualization documentation].
...
NOTE: Ensure that when you are moving from block storage in VMware, you have block storage class selected for the OpenShift Virtualization VM. Additionally, the volumeMode should be set to Block and access mode should be ReadWriteMany (RWX) so that you can perform live migration of the VM at a later time.
...
The dialog box shows the current time. Change the time to schedule the cutover for a later time, or click *Set cutover* to perform the cutover immediately.
