...
====
+
.. Create a Snapshot on the new source cluster (original destination) to capture any changes made during the failover period. This ensures that all changes are replicated back to the original source cluster during the resync process.
+
.Show example
[%collapsible]
====
...
.. Set up a replication relationship from the new source cluster (original destination) to the original source cluster, configuring values for the reverse direction.
+
.Show example
[%collapsible]
====
...
+
.. Promote the replication relationship from the new source cluster (original destination) to the original source cluster by updating the desiredState field in the AppMirrorRelationship CR to "Promoted".
+
.Show example
[%collapsible]
====
