...
[.lead]
In addition to the method of creating a VM using the OpenShift Virtualization UI,  are other methods to create VMs on OpenShift Virtualization. This section provides an overview of some of those methods.
...
You must have a VolumeSnapshotClass created that can then be used to create a VolumeSnapshot. This would have been create as part of the prerequisites for installing OpenShift Virtualization. You can refer to the link:osv-deployment.html[Section for installing OpenShift Virtualization] for details.
