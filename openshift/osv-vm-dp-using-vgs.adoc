...
[.lead]
Protect virtual machines in Red Hat OpenShift Virtualization by creating crash-consistent snapshots of multiple VM disks using Trident Volume Group Snapshots and NetApp ONTAP. This procedure guides you through configuring the environment, enabling the Volume Group Snapshot feature, taking group snapshots of VM disks, and restoring data to ensure rapid recovery and data integrity for critical workloads.
...
= Steps to demonstrate the use of VolumeGroupSnapshotClass to create snapshots of a group of volumes/disks of a VM and then restore each disk from the snapshots 

== Install OpenShift Cluster version 4.19 or later 
...
.Verify the Kubernetes version in OpenShift Cluster
[%collapsible%closed]
...
* This needs to be done after setting up the default storage class and Snapshot class using trident so that the golden images are made available as VolumeSnapshots in the cluster using Trident CSI.
...
== Create a VM with 3 disks. (a root disk and 2 additional disks)
