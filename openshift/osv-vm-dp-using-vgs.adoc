...
. Install OpenShift Cluster version 4.19 by following the instructions in the Red Hat documentation: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22#Install[Installing OpenShift Container Platform].
...
spec:
  ...
  credentials:
    name: tbc-iscsi-secret
+
. Create the iSCSI storage class using the following YAML definition.
+
[source,yaml]
# sc-iscsi.yaml
