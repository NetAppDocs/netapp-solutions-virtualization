...
As a prerequisite, you should have already created the Trident backend, the StorageClass and the volume snapshot class objects. You can refer to the link:osv-trident-install.html[Trident installation section] for details. 
...
To configure ontap-san StorageClass on the cluster see the link:osv-trident-install.html[Section for configuring StorageClass].
