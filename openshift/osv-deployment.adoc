...
.	A storage profile is automatically created for each  Trident Storage Class created and is automatically configured with clone strategy set to snapshot. Access Mode is set to ReadWriteMany.  There is no need for manual configuration. 
