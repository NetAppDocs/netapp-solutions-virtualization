...
== Step 2: Configure Trident for on-prem OpenShift clusters

Use the following examples to create a Trident backend and a StorageClass for your ONTAP configuration. Select the tab that matches your storage protocol: NAS for NFS-based file storage, iSCSI for iSCSI block storage, NVMe/TCP for high-performance NVMe over TCP block storage, or FC for Fibre Channel block storage. Replace the placeholder values with your cluster details.
...
== Step 3: Configure Trident for ROSA clusters using Amazon FSx

Use the following examples to create a Trident backend and a StorageClass for Amazon FSx for NetApp ONTAP storage. Select the tab that matches your storage protocol: NAS for NFS-based file storage or iSCSI for block storage. Replace the placeholder values with your Amazon FSx for NetApp ONTAP cluster details.
...
== Step 4: Create a Trident VolumeSnapshotClass
