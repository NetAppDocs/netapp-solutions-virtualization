...
.Steps
. Create the backend secret and backend configuration file.
+
[source,yaml]
----
cat tbc-nas.yaml
...
spec:
  ...
  credentials:
    name: tbc-nas-secret
----
. Create the StorageClass definition.
+
[source,yaml]
----
cat sc-nas.yaml
...
.Steps
. Create the VolumeSnapshotClass definition.
+
[source,yaml]
----
...
----
. Create the VolumeSnapshotClass in the cluster.
+
[source,bash]
----
...
.Steps
. Set the default VolumeSnapshotClass annotation.
+
...
. Delete any pre-existing DV and VolumeSnapshot objects.
+
[source,bash]
----
oc delete dv,VolumeSnapshot -n openshift-virtualization-os-images --selector=cdi.kubevirt.io/dataImportCron
