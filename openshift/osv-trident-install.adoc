...
+
.Delete any pre-existing DataVolume (DV) and VolumeSnapshot
[source,bash]
----
oc delete dv,VolumeSnapshot -n openshift-virtualization-os-images --selector=cdi.kubevirt.io/dataImportCron
----

For sample YAML files to create Trident objects for Amazon FSx for NetApp ONTAP storage on ROSA clusters and a VolumeSnapshotClass, see the sections later on this page.
...
== Configure Trident for ROSA clusters using Amazon FSx for NetApp ONTAP storage

Use the following examples to create a Trident backend and a StorageClass for Amazon FSx for NetApp ONTAP storage. Replace the placeholder values with your Amazon FSx for NetApp ONTAP cluster details.

.Trident backend and storage class for Amazon FSx for NetApp ONTAP NAS
...
type: Opaque
stringData:
  username: <FSx for ONTAP or SVM admin username, for example fsxadmin>
  password: <FSx for ONTAP or SVM admin password>
---
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
metadata:
...
.Trident backend and storage class for Amazon FSx for NetApp ONTAP iSCSI
