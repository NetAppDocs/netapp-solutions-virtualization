...
. The backend configuration files references your **ONTAP SVM** and **management LIF**. +
...
.iscsi SAN economy
--
Create a TridentBackendConfig and StorageClass for ONTAP SAN economy with iscsi to enable iscsi-based block storage provisioning. The backend configuration uses the ontap-san driver with the iscsi protocol and includes credentials stored in a Kubernetes Secret.
...
stringData:
  clientPrivateKey:
    <client private key that you generated>
---
apiVersion: trident.netapp.io/v1
kind: TridentBackendConfig
metadata:
...
spec:
  ...
  managementLIF: "10.192.102.47"
  #dataLIF: 10.61.180.32
  backendName: tbc_san_eco
  svm: oc-automation-svm #<replace with your SVM name>
  storagePrefix: oc_automation #<replace with your prefix>
  credentials:
    name: ontap-san-cert-auth
  clientCertificate:
    <client certificate that you generated>
