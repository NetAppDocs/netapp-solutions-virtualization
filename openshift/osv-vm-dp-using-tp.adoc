...
A simple way to restore only a specific file from the VM disks, is to make the snapshot directory on the volume that is backing the VM disks to be visible to the client. This can be done by editing the trident backend configuration to include the default section and set the snapshotDir parameter to true. This will automatically enable you to access the .snapshot directory in the client from the same mount location as the main volume. The .snapshot directory contains all the snapshots. You can change directory to the required snapshot, find the file you want to restore and copy it into the required location. 
NOTE: If you are using NFS version 4, you cannot see the .snapshot dir when you list files with an ls command. But you can cd to the .snapshot directory. 
