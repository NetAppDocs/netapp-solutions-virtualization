...
NOTE: This method exposes historical data to workloads/users with mount access and may violate least-privilege/compliance expectations. Please consider the security/operational impact, and note that you should enable it temporarily and revert it after restoring a file. Also, if you are using NFS version 4, you cannot see the `.snapshot` directory when you list files with an `ls` command. However, you can `cd` to the `.snapshot` directory.
